...
class PresentationService {
	...
	def showSlide(String conf, String room, String presentationName, String id) {
		def slide = new File(roomDirectory(conf, room).absolutePath + File.separatorChar + presentationName + File.separatorChar + "slide-${id}.swf")
		
		if (slide.exists()) {
			slide
		} else {
			def id_substract_one = id.toInteger() - 1
			new File(roomDirectory(conf, room).absolutePath + File.separatorChar + presentationName + File.separatorChar + "${id_substract_one}.swf")
		}
	}
	...
	def showThumbnail = {conf, room, presentationName, thumb ->
		...
		def thumbFile_ = new File(thumbFile)
		if (thumbFile_.exists()) {
			thumbFile_
		} else {
		    new File("/var/bigbluebutton/thumb.png")
		}
	}
	...
	def showTextfile = {conf, room, presentationName, textfile ->
		...
		def txtFile = new File(txt)
		if (txtFile.exists()) {
			txtFile
		} else {
			new File("/var/bigbluebutton/slide.txt")
		}
	}
	...
	def numberOfThumbnails = {conf, room, name ->
		...
		log.debug "numberOfSlide teststetss"


		if (thumbDir.exists()) { 
			thumbDir.listFiles().length
		} else {
			def docPath = roomDirectory(conf, room).absolutePath + File.separatorChar + name + '/documentData.txt'
        	def fileContent = new File(docPath).text
        	def docNameMatches = (fileContent =~ /noOfPages=([^\,]+)/);
        	def noSlides = docNameMatches[0][1].toInteger()
        	log.debug "numberOfSlide $noSlides"
        	noSlides
			//def presentationRootDir = new File(roomDirectory(conf, room).absolutePath + File.separatorChar + name)
			//presentationRootDir.listFiles().length
		}
	}
	...
	def numberOfTextfiles = {conf, room, name ->
		...
		if (textfilesDir.exists()) { 
			...
			def presentationRootDir = new File(roomDirectory(conf, room).absolutePath + File.separatorChar + name)
			presentationRootDir.listFiles().length - 2
		}
	}
