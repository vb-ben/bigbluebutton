...
*/
  ...
  def join = {
	...
	log.debug "Adapt dimdim presentations"
  	
  	log.debug "Create dir ${presentationService.presentationDir}/${meeting.getInternalId()}"
	try {
    	['mkdir', '-p', "${presentationService.presentationDir}/${meeting.getInternalId()}"].execute().waitFor()
    } catch (e) {
    	log.debug "Can not create dir ${presentationService.presentationDir}/${meeting.getInternalId()}"
    }
    
    def roomAbsolutePath = presentationService.roomDirectory(meeting.getInternalId(),meeting.getInternalId()).absolutePath
    
	// create symbolic link
    try {
    	['ln', '-s', roomAbsolutePath, "${presentationService.presentationDir}/${meeting.getInternalId()}/${meeting.getInternalId()}"].execute().waitFor()
    } catch (e) {
    	log.debug "Can not create symbolic link ${presentationService.presentationDir}/${meeting.getInternalId()}/${meeting.getInternalId()}"
    }
    
    // retrieve directory list
    def presentationsList = presentationService.listPresentations(meeting.getInternalId(), meeting.getInternalId())
    for (s in presentationsList) {
        if (s.contains("_____")) {
        	def presentationPath = roomAbsolutePath + '/' + s
        	try {
	        	def docPath = presentationPath + '/documentData.txt'
	        	def fileContent = new File(docPath).text
	        	def docNameMatches = (fileContent =~ /docName=([^\.]+)/);
	        	def newPresentationPath = roomAbsolutePath + '/' + docNameMatches[0][1]
	        	['mv', presentationPath, newPresentationPath].execute().waitFor()
	        } catch (e) {
	            log.debug e
	            ['rm', '-rf', presentationPath].execute().waitFor()
	        }
        }
    }
